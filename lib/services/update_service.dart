...
class UpdateService {
  ...
  // Dados padrão do servidor de atualização (adaptado para controle-patrimonio)
  static const String _defaultServerIp = "128.1.1.49";
  static const int _defaultServerPort = 8090;
  static const String _apkPath = "apk/controle-patrimonio.apk";
  ...
  Future<UpdateCheckResult> checkForUpdates() async {
    debugPrint('[UpdateService] ===== INICIANDO VERIFICAÇÃO DE ATUALIZAÇÕES =====');
    
    if (_isChecking || _isDownloading) {
      debugPrint('[UpdateService] Operação já em andamento');
    ...
    try {
      ...
      debugPrint('[UpdateService] Versão atual: $currentVersion+${packageInfo.buildNumber}');
      
      // Verificar se o servidor está online
      bool isServerOnline = false;
      String errorMessage = '';
      
      debugPrint('[UpdateService] Servidor configurado: $_serverUrl');
      debugPrint('[UpdateService] Tentando conectar ao servidor (timeout: ${_connectionTimeout.inSeconds}s, retries: $_connectionRetries)...');
      ...
      for (int i = 0; i < _connectionRetries; i++) {
        try {
          debugPrint('[UpdateService] Tentativa ${i + 1}/$_connectionRetries - GET $_serverUrl');
          ...
          debugPrint('[UpdateService] Resposta recebida: ${response.statusCode}');
          
          if (response.statusCode < 400) {
            isServerOnline = true;
            debugPrint('[UpdateService] ✓ Servidor online!');
            break;
          } else {
            errorMessage = 'Erro de servidor: ${response.statusCode}';
            debugPrint('[UpdateService] ✗ $errorMessage');
          }
        } catch (e) {
          errorMessage = e.toString();
          debugPrint('[UpdateService] ✗ Erro na tentativa ${i + 1}: $errorMessage');
        ...
        if (i < _connectionRetries - 1) {
          debugPrint('[UpdateService] Aguardando 800ms antes da próxima tentativa...');
      ...
      if (!isServerOnline) {
        _isChecking = false;
        debugPrint('[UpdateService] ===== SERVIDOR INDISPONÍVEL =====');
        debugPrint('[UpdateService] Erro final: $errorMessage');
        return UpdateCheckResult(
          status: UpdateStatus.serverUnavailable,
      ...
      try {
        debugPrint('[UpdateService] Buscando version.json...');
        // Criar um novo cliente para esta solicitação
        final versionClient = http.Client();
        try {
          debugPrint('[UpdateService] GET $_versionUrl');
          final response = await versionClient.get(
            Uri.parse(_versionUrl),
            headers: {'Connection': 'close'},
          ).timeout(_connectionTimeout);
          
          debugPrint('[UpdateService] version.json response: ${response.statusCode}');
